...
/**
 * @brief Gets the current time from DS1307 and returns it as time_t.
...
time_t DS1307_GetTime(void) {
    ...
    // Lấy thông tin về thời gian từ DS1307
    currentTime.tm_sec = DS1307_GetSecond();
    currentTime.tm_min = DS1307_GetMinute();
    currentTime.tm_hour = DS1307_GetHour();
    currentTime.tm_mday = DS1307_GetDate();
    currentTime.tm_mon = DS1307_GetMonth();
    currentTime.tm_year = DS1307_GetYear();
    currentTime.tm_wday = DS1307_GetDayOfWeek();
    // Chuyển đổi struct tm thành time_t
    timeValue = mktime(&currentTime);
 ...
 */
uint8_t DS1307_GetDayOfWeek(void) {
	return DS1307_DecodeBCD(DS1307_GetRegByte(DS1307_REG_DOW)) % 6;
}
